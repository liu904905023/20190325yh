 ...
 * */
    ...
    public function showratefee() {
        ...
        if (session('data')['CustomersType'] == 0 & $flag == 0) {
            if ($SysNo != 'null') {
                $url = C('SERVER_HOST') . "IPP3Order/IPP3Order_Group_ShopSPRate";
            }
            if ($CustomerSysNo != 'null') {
                $url = C('SERVER_HOST') . "IPP3Order/IPP3Order_Group_CustomerUserRateSP";
            }
        }
        //服务商员工
        if ($type == 0 & $flag == 1) {
        ...
        $totalcount = json_decode($list, true);
        $info['total']['Total_fee'] = fee2yuan($totalcount['Total_fee']);
        $info['total']['fee'] = fee2yuan($totalcount['fee']);
        $info['total']['Tradecount'] = $totalcount['Tradecount'];
        $info['total']['Rate_fee'] = fee2yuan($totalcount['Rate_fee']);
        $info['total']['Total_Rate_fee'] = fee2yuan($totalcount['Total_Rate_fee']);
        $this->ajaxreturn($info);
    }
